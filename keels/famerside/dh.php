...
$description =$_GET['username'];
                   ...
                   $sql = "select count(1) FROM farmers";
				  ...
				  $sql4 = "SELECT SUM(area) FROM oder  WHERE username= '$description'  ";
...
$sql1 = "SELECT COUNT(1) FROM oder  WHERE username= '$description' ";
...
<?php
                   ...
                   $sql6 = "SELECT   sum(price) FROM oder  WHERE username= '$description' ";
			...
			<div class="card-box pd-20 height-100-p mb-30">
				<div class="row align-items-center">
					<div class="col-md-4">
						...
						<img src="image/nsbm.png" alt="" class="sky1">
					</div>
					...
					<div class="col-md-8">
						...
						<p class="font-18 max-width-600" style=" font-family: 'Work Sans', sans-serif;
 font-weight: 300; ">Best this season only at Famers <br><span> Over 500+ Product with buy <span><br> More hight Rate. </p>
					</div>
				</div>
			</div>
			<div class="row">
					...
					<div class="card-box height-100-p widget-style1">
						<div class="d-flex flex-wrap align-items-center">
							...
							<div class="widget-data">
								...
								<div class="weight-600 font-14">MY Total Oder</div>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="card-box height-100-p widget-style1">
						<div class="d-flex flex-wrap align-items-center">
							...
							<div class="widget-data">
								...
								<div class="weight-600 font-14">MY Total Harvest Area(Fet)</div>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xl-3 mb-30">
					<div class="card-box height-100-p widget-style1">
						<div class="d-flex flex-wrap align-items-center">
							...
							<div class="widget-data">
							<div class="h4 mb-0"><h1>Rs<?php echo $total6;; ?>/-</h1></div>
            
								<div class="weight-600 font-14">Estimated Revenue</div>
							</div>
						</div>
					</div>
				</div>
...
<iframe src="map/admin-map.php" width="100%" height="100%" style="border:none;">
</iframe>
...
<div class="chat-popup" id="myForm">
  ...
  <form action="Chat.php" class="form-container">
	...
	$sql9 = "SELECT * FROM chat WHERE name = '$description' ";
	...
	while ($res9 = mysqli_fetch_array($result9)) {
		...
		{
			echo " <h7 id='a'>$masg</h7>";
			echo "<p id='a' ><i class='fa fa-check' aria-hidden='true' id='a' ></i><i class='fa fa-check' aria-hidden='true'></i> Sent</p>";
...
<?Php 
	...
	$sql40 = "SELECT * FROM chat WHERE name = '$description' ";
	$result40 = mysqli_query($conn, $sql40) or die( mysqli_error($conn));
 
 
	while ($res40 = mysqli_fetch_array($result40)) {
		...
		if($s == "1"){
			...
			echo "<h7 id='c'>$masg</h7>";
			echo "<p id='c' ><i class='fa fa-check' aria-hidden='true' id='c' ></i><i class='fa fa-check' aria-hidden='true'></i> Received</p>";
    ...
    <input placeholder="" name="name" value="<?php echo $description?>"  id="name1"   required>
...
<div class="card-box mb-30" style="margin-top: 2%;">
					...
					<tbody>
						<tr>
							...
							<?php
									...
									$sql = "SELECT * FROM  oder WHERE username= '$description' ";
									$result = mysqli_query($conn, $sql) or die( mysqli_error($conn));
									...
									while($res = mysqli_fetch_array($result)) {         
									 echo "<tr>";
									 echo "<td >".$res['location_status']."</td>";
									 ...
									 echo "<td  >".$res['id']."</td>"; 
									 echo "<td><a href=\"edit.php?id=$res[id]\">Edit</a> | <a href=\"delete.php?id=$res[id]&username=$description\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>"; 
...
var price = <?php  echo $total6?>;

var  p =  parseInt(price) / 1000;
...
var options5 = {
	...
	series: [{
		name: 'Food',
		data: [carrot, 28, 47, 22, 34, 25,7,8,7,8,9,7]
	}, {
